...
export default {
  async fetch(request) {
    ...
    const passageId = (new URL(request.url).searchParams.get("passageId") || "").toUpperCase().trim();
    if (!key) return Response.json({error:"API_BIBLE_KEY has not been added to Vercel."},{status:500});
    if (!VALID.test(passageId)) return Response.json({error:"Invalid New Testament passage reference."},{status:400});
